    ...
    try {
      ...
      if (!user) {
        return res.render("signin", {
          errors: { email: { msg: "Invalid email" } },
        });
      }

      if (user.isBlocked) {
        return res.render("signin", {
          errors: { email: { msg: "This email address is blocked." } },
        });
      }
...
exports.resetPasswordGet = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .render("resetpassword", { id, token, email: user.email });
  } catch (error) {
    console.error(error.message);
...
exports.shoppageGet = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .render("shop", { products, category, totalPages, currentPage: page });
  } catch (error) {
