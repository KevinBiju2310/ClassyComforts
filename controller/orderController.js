...
exports.orderPlaced = async (req, res) => {
  try {
    ...
    const address = userAddresses.items.find(
    ...
    if (paymentMethod === "cod") {
      if (totalValue >= 1000) {
        return res.json({
          success: false,
          message: "Above 1000 not allowed for Cash on Delivery",
        });
      }
      ...
      await Cart.findOneAndUpdate(
        { userId },
        {
          $pull: {
            products: {
              productId: { $in: checkedProducts.map((item) => item.productId) },
            },
          },
        }
      );
...
exports.generateInvoice = async (req, res) => {
  try {
    ...
    doc
      ...
      .fontSize(10)
      .moveDown()
      .text(`DATE: ${currentDate.toLocaleDateString("en-IN")}`, {
        align: "left",
      })
      .moveDown()
    ...
    order.address.forEach((address, index) => {
      doc
        .text(`Delivered To: ${address.name}`, { align: "right" })
    ...
    // Footer
    const footerText =
      "Thank you for your business, we appreciate your custom!";
    doc.fillColor("#333333").fontSize(10).text(footerText, { align: "center" });
    doc.moveDown();
    doc.moveDown();
  ...
  try {
    ...
    if (paymentMethod === "cod") {
      ...
      if (wallet) {
        ...
        const existingTransaction = wallet.transaction.find(
          (transaction) =>
...
exports.removeProduct = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({
        message: "Product removed from order and quantity updated",
        order,
      });
  } catch (error) {
