...
exports.dashboardGet = async (req, res) => {
  try {
    ...
    const topProductsData = topProducts.map((product) => {
      const productDetail = topProductDetails.find((p) =>
    ...
    res.render("dashboard", {
      ...
      monthlyEarning:
        monthlyEarning.length > 0 ? monthlyEarning[0].monthlyEarning : 0,
  ...
  try {
    if (interval === "monthly") {
      ...
      labels = Array.from({ length: 12 }, (_, i) =>
        moment(new Date(null, i)).format("MMM")
      );
...
exports.downloadPDF = async (req, res) => {
  try {
    ...
    doc
      .fontSize(8)
      .text(`Sales Count: ${overallSalesCount}`, { align: "right" });
    doc
      .fontSize(8)
      .text(`Discount Total: $${overallDiscountTotal.toFixed(2)}`, {
        align: "right",
      });
    doc
      .fontSize(8)
      .text(`Total Amount: $${overallTotalAmount.toFixed(2)}`, {
        align: "right",
      });
