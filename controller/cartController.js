...
exports.addtoCart = async (req, res) => {
  try {
    ...
    const existingProductIndex = cart
      ? cart.products.findIndex((item) => item.productId.equals(productId))
      : -1;
...
exports.updateCart = async (req, res) => {
  try {
    ...
    if (existingProductIndex !== -1) {
      ...
      let productPrice;
      if (
        cart.products[existingProductIndex].productId.mainprice <
        cart.products[existingProductIndex].productId.price
      ) {
        productPrice =
          cart.products[existingProductIndex].productId.mainprice !== 0
            ? cart.products[existingProductIndex].productId.mainprice
            : cart.products[existingProductIndex].productId.price;
      } else {
      ...
      const subtotal =
        cart.products[existingProductIndex].quantity * productPrice;

      cart.total = cart.products.reduce((acc, item) => {
        if (checkedProducts.includes(item.productId._id.toString())) {
          const price =
            item.productId.mainprice !== 0 &&
            item.productId.mainprice < item.productId.price
